...
export function excludeProperties(excludeArr, obj) {
  //*Loop through the array of keys to exclude
  excludeArr?.forEach((itemToExlude) => {
    //*Loop through the object keys and drop exlcuded keys value pairs
...
export function addRelationProperty(obj, parent, child) {
  ...
  //*Climb up the tree and perform calculations until the process is complete.
  while (!done) {
  ...
  obj[`${child} %${parent}`] = isNaN(relation)
    ? ""
    : relation === "Infinity"
    ? ""
